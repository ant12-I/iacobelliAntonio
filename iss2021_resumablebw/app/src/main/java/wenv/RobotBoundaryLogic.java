...
private   String command = "RESUME";
    ...
    public synchronized String doBoundaryInit(){
        ...
        System.out.println("RobotBoundaryLogic | doBoundary boundaruWalkDone1=" +boundaryWalkDone );
        while( ! boundaryWalkDone ) {
            try {
                System.out.println("RobotBoundaryLogic | doBoundary boundaruWalkDone=" +boundaryWalkDone );
                wait();
 ...
 //Business logic in RobotBoundaryLogic
    protected synchronized void boundaryStep( String move, boolean obstacle ){
        if (stepNum <= 4) {
            ...
            System.out.println("count "+ stepNum);
            robotInfo.showRobotMovesRepresentation();
        }else{ //stepNum > 4
